...
1. Now on the **Administrator** page click on **Your application** and copy the **Application name** value and save it locally in a text file, you will use these values in the later part of the lab.

   ![](media/img62.png) 

1. In the **Application** web page from the left-hand menu navigate to **Devices** and in the **All devices** tab copy the **Device ID** of **SN 100** device. Save the value locally in the text file, you will use these values in the later part of the lab.
...
1. On the device page click on the **Raw data** tab and under **Timestamp** verify the status of the device is **OFFLINE**.
...
1. On the **Rules** page click on **+ New** to add a rule. Based on which an action will be triggered by the **Logic app** to restart the sensor for which it will disable it first and enable it.
...
1. In the resource group tab select **iotcentrallogicapp(Deployment-ID)**.
...
1. Under the **Triggers** tab click on **When a rule is fired (preview)**. Then click on **Sign in**, when the sign in pop-up appears login with your credentials(you can find it from the environment details page), click on **Accept**.
...
1. In the **Execute a device command(preview)** page, select the following:
 ...
 1. Switch back to the where the application page is open, from the left navigation menu, select **Devices**. In the **Devices** page select **SN 100**, and navigate to the**Raw data** tab.
