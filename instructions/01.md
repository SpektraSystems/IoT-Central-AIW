...
1. From the top right corner click on the user icon, when prompted use the azure credentials provided in the **Environment Details** tab to log in to the Azure IoT Central portal.
...
1. Click on **Add**.  Under Telemetry select **Humidity**, click on the Settings button next to it and select **Aggregates**, and select **Minimum**.
...
1. On the righ-hand side you can see the chart control that visualizes the data as a line chart. You can toggle the visibility of specific lines by interacting with the chart legend.
